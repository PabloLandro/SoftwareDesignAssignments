...
Among the projects considered, the only ones that could be built easily without importing numberus dependencies were \texttt{Kafka} and \texttt{Guava}. However, since we were unable to run the SonarQube analysis on \texttt{Kafka}, we focused our study on \texttt{Guava} which allows us to use both tools.
